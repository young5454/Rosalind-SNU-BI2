...
def motif_to_profile_pseudocounts(motifs):
    t = len(motifs)
    k = len(motifs[0])
    total_nucleotides = []
    for i in range(k):
        curr_nucleotides = [1, 1, 1, 1]
        for motif in motifs:
            if motif[i] == 'A':
        ...
        curr_nucleotides = [x / (t + 4) for x in curr_nucleotides]
        total_nucleotides.append(curr_nucleotides)
    return np.array(total_nucleotides)


def GreedyMotifSearch_pseudocounts(dna, k, t):
    ...
    for i in range(text_length - k + 1):
        ...
        for j in range(2, t + 1):
            next_string = dna[j - 1]
            curr_prob_list = motif_to_profile_pseudocounts(kmer_list)
...
if __name__ == '__main__':
    with open("rosalind_ba2e.txt") as file:
        ...
        t = int(t)
        dna = data_list[1:]
        plits = GreedyMotifSearch_pseudocounts(dna, k, t)
        for d in plits:
            print(d)