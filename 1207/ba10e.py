...
def make_emission(alignments, checklist, profile, sigma):
    ...
    # # Get insertion node indices and probabilities
    # ins_numbering = []
    # for i in range(len(checklist)):
    #     if checklist[i] == 'i':
    #         # Count the number of h or d occurrences before i
    #         i_index = checklist[:i].count('h') + checklist[:i].count('d')
    #         ins_numbering.append(i_index)
    
    # ins_numbering = list(set(ins_numbering))
    # print(ins_numbering)
    ...
    if 'I0' in ins_dic.keys():
        ...
        all_dic_emission['I0'] = list(np.zeros(num_of_cols))
    ...
    for k in range(num_of_m):
        ...
        if m in hid_dic.keys():
            ...
            all_dic_emission[m] = list(np.zeros(num_of_cols))
        ...
        if i in ins_dic.keys():
            ...
            all_dic_emission[i] = list(np.zeros(num_of_cols))
    ...
    # for elem in all_dic_transition.items():
    #     key, value = elem
    #     listy = list(value.values())
    #     stringed = []
    #     for l in listy: 
    ...
    #             stringed.append('0')
    #         else:
    #             stringed.append(str(format_decimal(l)))
    #     row = key + '\t' + '\t'.join(stringed)
    #     transition.append(row)
...
if __name__ == '__main__':
    with open("rosalind_ba10e.txt") as file:
        # Theta value
        ...
        emission, transition = make_emission(alignments, checklist, profile, sigma)
        for t in transition:
            print(t)
        print('--------')
        for e in emission:
