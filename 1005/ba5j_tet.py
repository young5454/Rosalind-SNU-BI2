...
def affine_gap_alignment(seq1, seq2, n, m, gap_opening, gap_extension):
    ...
    for i in range(n):
        for j in range(m):
            ...
            lower_grid[i+1, j+1] = max(lower_x, lower_y)
 
            upper_x = upper_grid[i+1, j] - gap_extension
            upper_y = middle_grid[i+1, j] - gap_opening
            upper_grid[i+1, j+1] = max(upper_x, upper_y)
    ...
    return max_score, aligned_sequence1, aligned_sequence2, middle_grid, path_grid
...
if __name__ == '__main__':
    with open('rosalind_ba5j.txt', 'r') as file:
        ...
        print(affine_gap_alignment(seq1, seq2, n, m, gap_opening, gap_extension)[3])
        print(affine_gap_alignment(seq1, seq2, n, m, gap_opening, gap_extension)[4])