...
# pam250 as a nested dictionary data structure
...
def local_alignment(seq1, seq2, n, m):
    ...
    for i in range(n):
        for j in range(m):
            seq1_base = seq1[i]
            seq2_base = seq2[j]
            penalty = pam250[seq1_base][seq2_base]
    ...
    for k in range(n):
        for l in range(m):
            scores_list.append(grid[k+1, l+1])
    max_score = str(max(scores_list))

    # Get the index of the max_score
    max_i, max_j = unravel_index(grid.argmax(), grid.shape)
    seq1 = seq1[:max_i]
    seq2 = seq2[:max_j]
    ...
    return max_score, common_subsequence1, common_subsequence2, p, q
...
if __name__ == '__main__':
    with open('rosalind_ba5f.txt', 'r') as file:
        ...
        print(local_alignment(seq1, seq2, n, m)[0])
        print(local_alignment(seq1, seq2, n, m)[1])
        print(local_alignment(seq1, seq2, n, m)[2])
        # print(local_alignment(seq1, seq2, n, m)[3])
        # print(local_alignment(seq1, seq2, n, m)[4])